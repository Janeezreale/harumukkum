...
export default function FriendManageScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [friendsData, receivedData, sentData, todayPokedIds] =
        await Promise.all([
          getFriends(),
          getReceivedFriendRequests(),
          getSentFriendRequests(),
      ...
      setPokedIds(new Set(todayPokedIds.map((id) => String(id))));
    } catch (error) {
      console.log("Friend manage load failed", error);
    } finally {
      setIsLoading(false);
  ...
  function getUserStatus(userId: string): UserStatus {
    if (friends.some((f) => String(f.friend.id) === String(userId))) {
      return "friend";
    }

    if (sentRequests.some((r) => String(r.receiver.id) === String(userId))) {
      return "sent";
    }

    if (receivedRequests.some((r) => String(r.requester.id) === String(userId))) {
      return "received";
    }
  ...
  async function handleSendRequest(user: SearchUser) {
    setSendingIds((prev) => new Set(prev).add(String(user.id)));
    ...
    try {
      ...
      Alert.alert(
        "요청 완료",
        `${user.nickname ?? user.username}님에게 친구 요청을 보냈어요.`
      );
    } catch (e) {
  ...
  async function handlePoke(friendUserId: string) {
    ...
    if (pokedIds.has(friendId) || pokingIds.has(friendId)) {
      return;
    }
  ...
  function renderSearchResultAction(user: SearchUser) {
    ...
    const isSending = sendingIds.has(userId);
    ...
    if (status === "received") {
      const req = receivedRequests.find(
        (r) => String(r.requester.id) === userId
      );
            ...
            {sentRequests.map((req) => (
              <View key={req.id} style={styles.userCard}>
                <Image
                  source={{
                    uri:
                      req.receiver.profile_image_url ||
                      "https://via.placeholder.com/100",
                  }}
                  style={styles.avatar}
                />
        ...
        {friends.length > 0 ? (
          <>
            ...
            {friends.map((item) => {
              ...
              const isPoked = pokedIds.has(friendId);
              const isPoking = pokingIds.has(friendId);
              const isPokeDisabled = isPoked || isPoking;
              ...
              return (
                <View key={item.friendship_id} style={styles.userCard}>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.pokeBtn,
                      isPokeDisabled && styles.pokeBtnDone,
                    ]}
                    onPress={() => handlePoke(friendId)}
                  ...
                  >
                    {isPoking ? (
                      ...
                      <>
                        <Ionicons
                          name={isPoked ? "checkmark-circle" : "hand-left"}
                          size={13}
                        ...
                        <Text
                          style={[
                            styles.pokeBtnText,
                            isPoked && styles.pokeBtnTextDone,
                          ]}
                        >
                          {isPoked ? "찌름!" : "찌르기"}
                        </Text>
