...
export default function DiaryCalendarScreen() {
  const [activeTab, setActiveTab] = useState<typeof TABS[number]>('Routine & Emotion');
          ...
          {mockReport.insights.map((item) => {
            ...
            return (
              <View key={item.title} style={styles.insightRow}>
                <View style={[styles.insightIconCircle, { backgroundColor: item.color + '1A' }]}>
                ...
                <View style={styles.insightInfo}>
                  <View style={styles.insightLabelRow}>
                    ...
                    <Text style={[styles.insightDelta, { color: isPositive ? colors.positive : colors.negative }]}>
                      {isPositive ? '+' : ''}{item.delta_percent}%
                    </Text>
                  </View>
                  <View style={styles.insightBarBg}>
                    <View style={[styles.insightBarFill, { width: barWidth, backgroundColor: item.color }]} />
                  </View>
                </View>
              </View>
            );
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1 },
  scrollContent: { paddingHorizontal: 20, paddingBottom: 32, paddingTop: 16, gap: 16 },
  ...
  observationTitle: { fontSize: 15, fontWeight: '700', color: colors.primaryDark },
